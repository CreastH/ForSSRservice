...
Set_config_password(){
	...
	read -e -p -t 1 "(默认: doub.io):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="mianliu"
...
Set_config_method(){
 ...
 ${Tip} salsa20/chacha20-*系列加密方式，需要额外安装依赖 libsodium ，否则会无法启动ShadowsocksR !" && echo
	read -e -p -t 1 "(默认: 5. aes-128-ctr):" ssr_method
...
Set_config_protocol(){
 ...
 ${Tip} 如果使用 auth_chain_a 协议，请加密方式选择 none，混淆随意(建议 plain)" && echo
	read -e -p -t 1 "(默认: 2. auth_sha1_v4):" ssr_protocol
	...
	if [[ ${ssr_protocol} != "origin" ]]; then
		if [[ ${ssr_protocol} == "auth_sha1_v4" ]]; then
			read -e -p -t 1 "是否设置 协议插件兼容原版(_compatible)？[Y/n]" ssr_protocol_yn
...
Set_config_obfs(){
 ...
 如果你是在日本、美国等热门地区搭建，那么选择 plain 混淆可能被墙几率更低 !" && echo
	read -e -p -t 1 "(默认: 1. plain):" ssr_obfs
	...
	if [[ ${ssr_obfs} != "plain" ]]; then
			read -e -p -t 1 "是否设置 混淆插件兼容原版(_compatible)？[Y/n]" ssr_obfs_yn
...
Set_config_protocol_param(){
	...
	read -e -p -1 "(默认: 无限):" ssr_protocol_param
...
Set_config_speed_limit_per_con(){
	...
	read -e -p -t 1 "(默认: 无限):" ssr_speed_limit_per_con
...
Set_config_speed_limit_per_user(){
	...
	read -e -p -t 1 "(默认: 无限):" ssr_speed_limit_per_user
...
cd /root/
wget -N --no-check-certificate "https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/ssr.sh" && chmod +x ssr.sh
wget -N --no-check-certificate "https://gist.github.com/zeruns/a0ec603f20d1b86de6a774a8ba27588f/raw/4f9957ae23f5efb2bb7c57a198ae2cffebfb1c56/tcp.sh" && chmod +x tcp.sh
